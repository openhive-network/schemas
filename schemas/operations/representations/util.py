...
__hf26_operation_representations: dict[str, type[HF26Representation]] = {}
__legacy_operation_representations: dict[str, type[LegacyRepresentation]] = {}


def get_hf26_operation_representation(type_name: str) -> type[HF26Representation]:
    return __get_representation_from_type_dict(type_name, __hf26_operation_representations)


def get_legacy_operation_representation(type_name: str) -> type[LegacyRepresentation]:
...
def _create_hf26_representation(incoming_type: type[Operation]) -> type[HF26Representation]:
    ...
    Hf26Operation = create_model(  # type: ignore[call-overload]   # noqa: N806
        new_model_name, __base__=HF26Representation, **field_definitions
    )
    __hf26_operation_representations[incoming_type.get_name()] = Hf26Operation
    return Hf26Operation  # type: ignore[no-any-return]


def _create_legacy_representation(incoming_cls: type[Operation]) -> type[LegacyRepresentation]:
    """
    ...
    LegacyOperation = create_model(  # type: ignore[call-overload]   # noqa: N806
        new_model_name, __base__=LegacyRepresentation, **field_definitions
    )
