...
class AssetBase(Serializable, ABC):
    ...
    def copy(self, *, amount: Any | int | AssetBase | None = None) -> Self:
        if amount is None:
    ...
    @classmethod
    def parse_raw(
        cls, str_to_parse: str
    ) -> tuple[AssetNaiAmount, HiveInt, str]:  # *, use_asset_info: AssetInfo | None = None) -> tuple[int, int, str]:
    ...
    def token(self, testnet: bool | None = None) -> str:
        return self.get_asset_information().get_symbol(testnet=(testnet or self._is_default_symbol_testnet()))
