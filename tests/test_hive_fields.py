...
class AuthorityModel(msgspec.Struct):
    field: Authority
...
@pytest.mark.parametrize(
    ...
    [
        ("definitely too long name", "Expected `str` of length <= 16 - at `$.field`"),
        ("to", "Expected `str` of length >= 3 - at `$.field`"),
        (
            "123",
            "Expected `str` matching regex '^[a-z]{1}[a-z0-9\\\\-]+[a-z0-9]{1}(:?\\\\.{1}[a-z]{1}[a-z0-9\\\\-]+[a-z0-9]{1})*$' - at `$.field`",
        ),
    ],
)
def test_account_name_incorrect_value(value: str, expected_message: str) -> None:
    ...
    with pytest.raises(Exception) as error:
        msgspec.json.decode(account_name, type=AccountNameModel)
...
def test_asset_nai_vests_field_incorrect_amount(value: str) -> None:
    ...
    # ACT
    with pytest.raises(Exception) as error:
        AssetVests(amount=HiveInt(value), precision=HiveInt(6), nai="@@000000037")  # type: ignore[arg-type]
...
@pytest.mark.parametrize(
    ...
    [
        (
            POSTING,
            "Not_int",
            "The value could only be int or string that can be converted to int! - at `$.field.weight_threshold`",
        ),
        (
            OWNER,
            "Bad_account_name",
            "Expected `str` matching regex '^[a-z]{1}[a-z0-9\\\\-]+[a-z0-9]{1}(:?\\\\.{1}[a-z]{1}[a-z0-9\\\\-]+[a-z0-9]{1})*$' - at `$.field.account_auths[0][0]`",
        ),
        (
            ACTIVE,
            "SMT@@",
            "Expected `str` matching regex '^(?:STM)[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{7,51}$' - at `$.field.key_auths[0][0]",
        ),
    ],
)
...
def test_public_key_field_incorrect_values(value: str) -> None:
    ...
    expected_message: Final[
        str
    ] = "Expected `str` matching regex '^(?:STM)[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{7,51}$' - at `$.field`"
...
def test_correct_value_asset_hive_legacy() -> None:
    ...
    # ACT
    decoded: AssetHiveLegacyModel = decoder.decode(asset_hive_legacy_model)
