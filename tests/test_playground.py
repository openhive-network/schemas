...
def enc_hook(value):
    if isinstance(value, (AssetHbd | AssetHive | AssetVests)):
...
x = LegacyRepresentationClaimRewardBalanceOperation(
    value=ClaimRewardBalanceOperation(
        account="bob", reward_hive=AssetHive(amount=0), reward_hbd=AssetHbd(amount=0), reward_vests=AssetVests(amount=1)
    )
)
...
def test_account_history_api_correct_values() -> None:
    encoder.encode(x)
    # ARRANGE
    json.dumps(GET_TRANSACTION, sort_keys=True)

    # ACT
    get_response_model(GetTransaction, json.dumps(GET_TRANSACTION))
